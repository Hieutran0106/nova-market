import React,{useState} from 'react';import {Link} from 'react-router-dom';import {ChevronRight,Eye,EyeOff,Gift,Heart,LockKeyhole,LogOut,Mail,Package,Phone,ShieldCheck,Truck,UserRound} from 'lucide-react';import {promotions} from '../data/store';import ProductCard from '../components/ProductCard';import {useStore} from '../context/StoreContext';

export function Promotions(){
const { products } = useStore();
const deals=[...products].sort((a,b)=>b.discount-a.discount).slice(0,8);
return <><section className="promo-hero"><div className="container"><span>ĐẠI TIỆC KHUYẾN MẠI</span><h1>GIỜ VÀNG GIÁ SỐC</h1><p>Deal mới mỗi ngày · Số lượng có hạn · Hàng chính hãng</p><Link className="btn primary" to="/products">Săn deal ngay</Link></div></section><div className="container promo-page"><div className="promo-cards">{promotions.map((x,i)=><article key={i} className={x.className}><span>{x.icon}</span><div><h3>{x.title}</h3><p>{x.desc}</p></div></article>)}</div><section className="promo-products"><div className="promo-section-head"><div><span>FLASH SALE</span><h2>Giảm sâu hôm nay</h2></div><Link to="/products">Xem tất cả →</Link></div><div className="product-grid four">{deals.map(product=><ProductCard key={product.id} p={product}/>)}</div></section></div></>}

export function Wishlist(){const {wishlist, products}=useStore();const list=products.filter(p=>wishlist.includes(p.id));return <div className="container generic-page"><h1>Sản phẩm yêu thích</h1>{list.length?<div className="product-grid four">{list.map(p=><ProductCard key={p.id} p={p}/>)}</div>:<div className="empty">Bạn chưa có sản phẩm yêu thích.</div>}</div>}

export function Compare(){const {compare, products}=useStore();const list=products.filter(p=>compare.includes(p.id));return <div className="container generic-page"><h1>So sánh sản phẩm</h1>{list.length<2?<div className="empty">Hãy chọn ít nhất 2 sản phẩm để so sánh.</div>:<div className="compare-table"><div></div>{list.map(p=><b key={p.id}>{p.name}</b>)}{['price','rating','stock'].map(k=><React.Fragment key={k}><span>{k==='price'?'Giá bán':k==='rating'?'Đánh giá':'Tồn kho'}</span>{list.map(p=><div key={p.id}>{k==='price'?new Intl.NumberFormat('vi-VN').format(p.price)+'đ':k==='rating'?p.rating:p.stock+' sản phẩm'}</div>)}</React.Fragment>)}</div>}</div>}
